...
const DateStrip = ({ plans, onDateClick }) => {
    ...
    const todayBorder = 'ring-2 ring-[#f97316] ring-offset-2 ' + (isDarkMode ? 'ring-offset-[#121212]' : 'ring-offset-white');
    ...
    return (
        <div className="flex-1 w-full overflow-hidden px-4 relative">
            {/* Left fade mask */}
            <div className={`absolute left-0 top-0 bottom-0 w-8 z-10 bg-linear-to-r ${isDarkMode ? 'from-[#121212]' : 'from-[#3b82f6]'} to-transparent pointer-events-none`} />
            ...
            {/* Right fade mask */}
            <div className={`absolute right-0 top-0 bottom-0 w-8 z-10 bg-linear-to-l ${isDarkMode ? 'from-[#121212]' : 'from-[#3b82f6]'} to-transparent pointer-events-none`} />
        </div>
    );
};
