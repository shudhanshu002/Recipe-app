...
const DateStrip = ({ plans, onDateClick }) => {
  ...
  // Group plans by date to find the first recipe image for each active day
  ...
  // Determine which item is "Today" or the nearest Future date
  ...
  const scrollToIndex =
    initialIndex !== -1 ? initialIndex : Math.max(0, sortedDates.length - 1);
  ...
  const todayBorder =
    ...
    (isDarkMode ? 'ring-offset-[#121212]' : 'ring-offset-white');
  ...
  return (
    <div className="flex-1 w-full overflow-hidden px-4 relative">
      {/* Left fade mask */}
      <div
        className={`absolute left-0 top-0 bottom-0 w-8 z-10 bg-linear-to-r ${isDarkMode ? 'from-[#121212]' : 'from-[#3b82f6]'} to-transparent pointer-events-none`}
      />
        ...
        {slots.map((item, index) => {
          ...
          return (
            <div
              ...
              onClick={() =>
                !item.empty && onDateClick && onDateClick(item.date)
              }
              className={`
                                relative w-14 h-14 rounded-lg border ${borderColor} overflow-hidden shrink-0 
      ...
      {/* Right fade mask */}
      <div
        className={`absolute right-0 top-0 bottom-0 w-8 z-10 bg-linear-to-l ${isDarkMode ? 'from-[#121212]' : 'from-[#3b82f6]'} to-transparent pointer-events-none`}
      />
    </div>
  );
};
