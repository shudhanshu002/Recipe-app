...
const RecipeCard = ({ recipe, onClick, isActive, onUnbookmark }) => {
    const { user } = useAuthStore();
    const { isDarkMode } = useThemeStore();
    const navigate = useNavigate();

    const [isLiked, setIsLiked] = useState(recipe.isLiked || false);
    const [likeCount, setLikeCount] = useState(recipe.likesCount || 0); // Default to 0
    ...
    const toggleLike = async (e) => {
        ...
        setIsLiked(!isLiked);
        setLikeCount((prev) => (!previousLiked ? prev + 1 : Math.max(0, prev - 1)));
    ...
    const toggleBookmark = async (e) => {
        ...
        try {
            ...
            if (prev === true && onUnbookmark) {
                onUnbookmark(recipe._id);
            }
        } catch {
