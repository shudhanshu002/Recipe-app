...
const BlogCard = ({ blog, onDelete }) => {
  ...
  const isOwner =
    user &&
    (user._id === blog.author?._id || user.username === blog.author?.username);
      ...
      <>
        <div className="flex gap-4 mb-6 items-start group font-dancing">
          ...
          >
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span
                    className="inline-block text-[10px] px-2 py-0.5 rounded-full text-white font-bold"
              ...
              <h2 className={`text-lg font-bold mb-1 ${textColor}`}>
                {blog.title}
              </h2>
              <p
                className={`text-sm mb-2 ${subText} line-clamp-3 whitespace-pre-line`}
              >
                {stripHtml(blog.content)}
              </p>
              {blog.coverImage && (
                <div
                  className={`mt-3 h-32 w-full overflow-hidden rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
                >
                  <img
                    src={blog.coverImage}
                    alt="Cover"
            ...
            <div
              className={`flex justify-end items-center gap-4 px-5 py-3 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-100'} relative z-10`}
            >
              <div
