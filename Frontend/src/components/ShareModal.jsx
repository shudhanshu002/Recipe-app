...
const ShareModal = ({ url, title, onClose }) => {
    ...
    // Styles
    const modalBg = isDarkMode ? 'bg-[#1e1e1e] border-gray-700' : 'bg-white border-gray-200';
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    {shareLinks.map((link) => (
                        <a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2 group">
                            ...
                            <span className={`text-xs font-medium ${subText}`}>{link.name}</span>
                        </a>
                    ))}
                </div>

                {/* Copy Link Section */}
                <div className={`flex items-center gap-2 p-2 rounded-lg border ${isDarkMode ? 'bg-[#2d2d2d] border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                    <input type="text" readOnly value={url} className={`flex-1 bg-transparent text-sm outline-none px-2 truncate ${subText}`} />
                    <button
                        onClick={handleCopy}
