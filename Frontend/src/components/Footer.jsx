...
const Footer = () => {
  ...
  const iconBg = isDarkMode
    ? 'bg-gray-800 hover:bg-[#f97316] text-white'
    : 'bg-gray-100 hover:bg-[#f97316] hover:text-white text-gray-600';
  ...
  return (
    <footer
      className={`mt-auto relative pt-16 pb-8 px-6 overflow-hidden ${bgClass} rounded-t-[3rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]`}
    >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="space-y-4">
            <Link to="/" className="flex items-center gap-2 mb-2 group">
              ...
              <span
                className={`text-2xl font-bold font-dancing ${headingColor}`}
              >
                Zaika Vault
              </span>
            </Link>
            <p className="text-sm leading-relaxed max-w-xs">
              Discover, share, and organize your favorite recipes. We are
              building a community where flavor meets friendship.
            </p>
          ...
          <div>
            <h4
              className={`text-xl font-bold mb-6 font-dancing ${headingColor}`}
            >
              Discover
            </h4>
          ...
          <div>
            ...
            <p className="text-sm mb-4">
              Follow us on social media for daily recipes and behind the scenes.
            </p>
        ...
        {/* DIVIDER */}
        <div
          className={`h-px w-full bg-linear-to-r  mb-8 ${isDarkMode ? 'from-transparent via-gray-700 to-transparent' : 'from-transparent via-gray-300 to-transparent'}`}
        ></div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-medium">
          ...
          <p
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
          >
            Made with{' '}
            <Heart
              size={12}
            ...
            />{' '}
            by <span className={headingColor}>Sudhanshu</span>
          </p>
        </div>
      </div>
    </footer>
...
const FooterLink = ({ to, label }) => (
  <li>
    <Link
      to={to}
      className="group flex items-center gap-2 hover:text-[#f97316] transition-all duration-300"
    >
...
const SocialButton = ({ href, icon, classes }) => (
  <a
    href={href}
    className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-sm ${classes}`}
