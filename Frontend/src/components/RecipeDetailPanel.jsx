...
const RecipeDetailPanel = ({ recipeId, onClose }) => {
    ...
    const fetchRecipe = async () => {
      ...
      try {
        ...
        if (isForbidden) {
          ...
          if (isUserPremium) {
            toast.error(
              'Server verification failed. Please refresh your session.',
              {
  ...
  const panelBg = isDarkMode
    ? 'bg-[#1e1e1e] border-l border-gray-800'
    ...
    <div className={`h-full flex flex-col ${panelBg}`}>
      ...
      >
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className={`p-2 rounded-lg transition-colors ${iconHover} ${subText}`}
            title="Close"
          >
            <X size={20} />
          </button>
          <Link
            to={`/recipes/${recipeId}`}
            className={`p-2 rounded-lg transition-colors flex items-center gap-2 ${iconHover} ${subText}`}
      ...
      <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
        ...
        {!loading && fetchError === 'ACCESS_DENIED' && (
          <div className="flex flex-col items-center justify-center py-10 text-center h-full animate-in fade-in zoom-in-95">
            <div
              className={`p-6 rounded-full mb-6 ${isDarkMode ? 'bg-red-900/20' : 'bg-red-100'}`}
            >
            ...
            <div
              className={`mb-8 max-w-sm mx-auto p-4 rounded-xl border ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}
            >
              {isUserPremium ? (
                <>
                  ...
                  <p className={`${subText} text-sm`}>
                    You are logged in as Premium, but the server is blocking
                    your request.
                  </p>
                  <p className={`${subText} text-xs mt-2 opacity-75`}>
                    (Your Auth Token is outdated. Please refresh.)
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className={`${subText} text-sm`}>
                    This recipe is locked for free users.
                  </p>
                </>
              )}
            </div>
            ...
            {isUserPremium ? (
              ...
              <Link
                to="/subscription"
                className="flex items-center gap-2 bg-yellow-500 text-white px-8 py-3 rounded-xl font-bold hover:bg-yellow-600 transition shadow-lg"
              >
        ...
        {!loading && fetchError === 'GENERAL_ERROR' && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button
              ...
              className="mt-4 text-[#f97316] font-medium hover:underline"
            >
              Reload Page
            </button>
          </div>
        ...
        {!loading && recipe && (
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="aspect-video w-full rounded-xl overflow-hidden relative bg-gray-200 shadow-md">
              <img
                src={recipe.images?.[0] || 'https://via.placeholder.com/800'}
                alt={recipe.title}
              ...
              {isLocked && (
                <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-white backdrop-blur-md px-4 text-center">
                  ...
                  {isUserPremium ? (
                    <>
                      <span className="font-bold text-xl text-yellow-400 mb-1">
                        Sync Required
                      </span>
                      <p className="text-sm text-gray-200 mt-1 mb-6 max-w-xs mx-auto">
                        Your app says Premium, but the server token is old.
                      </p>
                      <button
                        onClick={handleRefreshSession}
            ...
            <div>
              ...
              <div
                className={`flex flex-wrap items-center gap-4 text-sm mt-4 pb-4 border-b ${subText} ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}
              >
                ...
                <span
                  className={`px-2 py-0.5 rounded text-xs font-medium ${tagBg}`}
                >
                  {recipe.difficulty}
                </span>
              </div>
            ...
            <div>
              ...
              {isLocked ? (
                <div
                  className={`p-8 rounded-xl text-center border-2 border-dashed ${isDarkMode ? 'border-gray-800 bg-gray-900/50' : 'border-gray-200 bg-gray-50'}`}
                >
                  {isUserPremium ? (
                    <>
                      <p className={`text-sm mb-4 font-medium ${subText}`}>
                        Instructions hidden by backend security rule.
                      </p>
                      <button
                        onClick={handleRefreshSession}
