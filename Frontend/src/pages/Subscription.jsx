...
const Subscription = () => {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // 1. Get Key
            const { data: keyResponse } = await axios.get(`${API_URL}/razorpay-key`, { withCredentials: true });
            const RAZORPAY_KEY_ID = keyResponse.data.key;

            // 2. Create Order (Send PLAN TYPE)
            const { data: orderResponse } = await axios.post(`${API_URL}/create-order`, { planType: selectedPlan }, { withCredentials: true });
    ...
    return (
        <div className={`font-dancing min-h-screen py-20 px-4 ${bgClass} rounded-4xl`}>
            ...
            <div className="flex justify-center mb-8">
                <div className={`p-1 rounded-full border ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-100'} flex`}>
                    <button
            ...
            <div className="max-w-md mx-auto relative">
                <div className={`rounded-2xl p-8 border-2 ${selectedPlan === 'yearly' ? 'border-yellow-500' : 'border-gray-300'} shadow-xl ${cardBg} transition-all`}>
                    {selectedPlan === 'yearly' && (
                    ...
                    <h2 className="text-2xl font-bold mb-2 capitalize">{selectedPlan} Plan</h2>
                    <div className="flex items-end gap-1 mb-6">
                        <span className="text-5xl font-bold">{selectedPlan === 'monthly' ? '₹499' : '₹4999'}</span>
                        <span className="text-gray-500 mb-1">/ {selectedPlan === 'monthly' ? 'month' : 'year'}</span>
                    </div>
                    ...
                    {user?.isPremium ? (
                        <div className="text-center">
                            <button disabled className="w-full py-4 rounded-xl bg-green-600 text-white font-bold flex items-center justify-center gap-2 cursor-default mb-2">
                                <ShieldCheck /> Active Subscription
                            </button>
                            <p className="text-sm text-gray-500 mt-2">
                                Expires on: <span className="font-semibold text-yellow-500">{formatDate(user.subscriptionExpiry)}</span>
                            </p>
                        </div>
                    ) : (
                        <button
                            ...
                            className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-105 ${
                                selectedPlan === 'yearly' ? 'bg-yellow-500 hover:bg-yellow-600 text-black' : 'bg-gray-800 text-white hover:bg-gray-900'
                            }`}
                        >
                            {loading ? <Loader className="animate-spin" /> : `Get ${selectedPlan} Premium`}
                        </button>
                    )}
                </div>
            </div>
