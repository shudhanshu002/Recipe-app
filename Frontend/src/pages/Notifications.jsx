...
const Notifications = () => {
    ...
    useEffect(() => {
        ...
        if (search.trim()) {
            ...
            result = result.filter((n) => n.sender?.username.toLowerCase().includes(q) || n.recipe?.title.toLowerCase().includes(q));
        }
    ...
    const getMessage = (n) => {
        ...
        switch (n.type) {
            ...
            case 'FOLLOW':
                return (
                    <span>
                        <span className="font-bold">{senderName}</span> started following you.
                    </span>
                );
            case 'COMMENT':
                return (
        ...
        <div className="max-w-3xl mx-auto space-y-6 mb-20">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
                {['ALL', 'LIKE', 'COMMENT', 'FOLLOW'].map((type) => (
                    ...
                    >
                        {type === 'ALL' ? 'All' : type === 'LIKE' ? 'Likes' : type === 'COMMENT' ? 'Comments' : 'Followers'}
                    </button>
                ))}
            </div>

            <div className={`rounded-xl shadow-sm border divide-y ${cardBg} ${isDarkMode ? 'divide-gray-700' : 'divide-gray-100'} overflow-hidden`}>
                        ...
                        <div key={notif._id} className={`p-4 flex items-start gap-4 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50`}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    {getIcon(notif.type)}
                                    <p className={`text-sm ${textColor} truncate`}>{getMessage(notif)}</p>
                                </div>
                                <span className={`text-xs ${subText}`}>{formatRelativeDate(notif.createdAt)}</span>
                            </div>
                        </div>
                    ))
                ) : (
