...
const AIChatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'system',
      content: "Hi! I'm ZaikaBot 👨‍🍳. Ask me anything about cooking!",
    },
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      const context = location.pathname.includes('/recipes/')
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'system', content: 'My oven is overheating! Try again later.' },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div
          className={`w-80 sm:w-96 h-[500px] rounded-2xl shadow-2xl border flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300 ${cardBg}`}
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                ...
                <div
                  className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                    msg.role === 'user'
                      ...
                      : `${isDarkMode ? 'bg-[#2d2d2d] text-gray-200' : 'bg-gray-100 text-gray-800'} rounded-bl-none`
                  }`}
                >
                  {msg.content}
                </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div
                  className={`px-4 py-2 rounded-2xl rounded-bl-none flex items-center gap-2 text-xs ${isDarkMode ? 'bg-[#2d2d2d] text-gray-400' : 'bg-gray-100 text-gray-500'}`}
                >
                  <Loader2 size={14} className="animate-spin" /> Chef is
                  thinking...
                </div>
              </div>
            )}
