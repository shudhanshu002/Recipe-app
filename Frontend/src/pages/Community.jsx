...
import { Search, Filter, X, Leaf, Drumstick, Clock, ChefHat, ArrowUpDown } from 'lucide-react';


// --- FILTER OPTIONS ---  based on differnt types
const CUISINES = ['All', 'Indian', 'Italian', 'Mexican', 'Chinese', 'American', 'Thai', 'Japanese', 'Mediterranean'];
const DIETS = [
    { label: 'All', value: 'all' },
...
const Community = () => {
    ...
    }, [page, debouncedSearch, selectedCuisine, selectedDiet, selectedTime, sortBy]);
    ...
    return (
        <div className={`min-h-screen ${bgClass} flex flex-col relative pb-20`}>
            ...
            {/* HEADER & FILTERS */}
            <div className={`font-dancing sticky top-16 z-30 px-4 py-4 border-b backdrop-blur-md transition-colors ${isDarkMode ? 'bg-[#121212]/95 border-gray-800' : 'bg-gray-100 border-gray-200'}`}>
                <div className={`max-w-7xl mx-auto space-y-4 `}>
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className={`text-2xl font-bold ${textColor}`}>Community Recipes</h1>
                            <p className={`text-xs ${subText}`}>Discover what others are cooking</p>
                        </div>
                    ...
                    {/* Filter Bar */}
                    <div className={`${showFilters ? 'flex' : 'hidden'} md:flex flex-col md:flex-row gap-3 md:items-center pb-2 flex-wrap`}>
                        {/* Diet Dropdown */}
                        <FilterDropdown
                            icon={selectedDiet === 'veg' ? Leaf : selectedDiet === 'non-veg' ? Drumstick : Filter}
                            label="Diet"
                            value={selectedDiet}
                            options={DIETS}
                        ...
                        {/* Cuisine Dropdown */}
                        <FilterDropdown icon={ChefHat} label="Cuisine" value={selectedCuisine} options={CUISINES} onChange={setSelectedCuisine} isDarkMode={isDarkMode} />

                        {/* Time Dropdown */}
                        <FilterDropdown icon={Clock} label="Cooking Time" value={selectedTime} options={TIMES} onChange={setSelectedTime} isDarkMode={isDarkMode} />

                        {/* Sort Dropdown */}
                        <FilterDropdown icon={ArrowUpDown} label="Sort By" value={sortBy} options={SORTS} onChange={setSortBy} isDarkMode={isDarkMode} />

                        {/* Clear Button */}
                        {(search || selectedCuisine !== 'All' || selectedDiet !== 'all' || selectedTime || sortBy) && (
                            <button onClick={clearFilters} className="text-sm text-red-500 hover:underline flex items-center gap-1 ml-auto md:ml-0">
                                <X size={14} /> Clear
                            </button>
                        )}
                    </div>
                ...
                <div className="flex flex-1 overflow-hidden gap-6 relative h-full">
                    ...
                    <div className={`flex-1 transition-all duration-300 ${selectedRecipeId ? 'hidden md:block md:w-1/2 lg:w-3/5' : 'w-full'}`}>
                        {recipes.length === 0 && !loading ? (
                            <div className={`flex flex-col items-center justify-center py-40 rounded-3xl border-2 border-dashed ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                                <Filter className="w-16 h-16 text-gray-300 mb-4" />
                                <h3 className={`text-xl font-bold mb-2 ${textColor}`}>No recipes found</h3>
                                <p className={subText}>Try adjusting your filters.</p>
                                <button onClick={clearFilters} className="mt-6 text-[#f97316] font-bold hover:underline">
                                    Reset Filters
                                </button>
                            </div>
                        ) : (
                            <div className={`grid gap-6 ${selectedRecipeId ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>
                                {recipes.map((recipe, index) => {
                                    ...
                                    if (recipes.length === index + 1) {
                                        return (
                                            <div ref={lastRecipeRef} key={recipe._id} className="contents">
                                                <RecipeCard recipe={recipe} onClick={() => handleRecipeClick(recipe)} isActive={selectedRecipeId === recipe._id} />
                                            </div>
                                        );
                                    } else {
                                        return <RecipeCard key={recipe._id} recipe={recipe} onClick={() => handleRecipeClick(recipe)} isActive={selectedRecipeId === recipe._id} />;
                                    }
                                })}
                            </div>
                        )}
