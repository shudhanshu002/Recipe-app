...
const MealPlanner = () => {
    ...
    const todayBg = isDarkMode ? 'bg-orange-900/20 border-orange-900/30' : 'bg-orange-50 border-orange-200';
    ...
    const groupedHistory = historyPlan.reduce((groups, meal) => {
        const date = new Date(meal.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
        ...
        <div className="max-w-7xl mx-auto space-y-8 mb-20 font-dancing">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
                ...
                {view === 'week' && (
                    <div className={`shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-lg border ${cardBg}`}>
                        ...
                        <button onClick={handleNextWeek} className={`p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${textColor}`}>
                            <ChevronRight />
                        </button>
                    </div>
                )}
                                        ...
                                        <div className="flex-1 space-y-2 overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                            {meals.map((plan) => (
                                                ...
                                                >
                                                    ...
                                                    <Link to={`/recipes/${plan.recipe?._id}`} className="flex gap-2 items-center">
                                                        <img src={plan.recipe?.images?.[0] || 'https://placehold.co/100?text=Food'} className="w-8 h-8 rounded object-cover bg-gray-300" alt="Thumb" />
                                                        <span className={`text-xs font-medium line-clamp-2 ${textColor}`}>{plan.recipe?.title || 'Unknown Recipe'}</span>
                                                    </Link>
                                                </div>
                                            ))}
                                        </div>
                        ...
                        <div className="space-y-6">
                            {Object.keys(groupedHistory).length > 0 ? (
                                Object.entries(groupedHistory).map(([date, meals]) => (
                                    <div key={date} className={`p-6 rounded-2xl border ${cardBg}`}>
                                        <h3 className={`text-lg font-bold mb-4 ${textColor}`}>{date}</h3>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                            {meals.map((plan) => (
                                                ...
                                                >
                                                    ...
                                                    <button onClick={() => handleDelete(plan._id, true)} className="p-2 text-gray-400 hover:text-red-500">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            ))}
                                ...
                                <div className={`text-center py-20 rounded-xl border border-dashed ${cardBg} ${subText}`}>No past meal history found.</div>
                            )}
                        </div>
                    )}
                </>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className={`w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[80vh] ${modalBg}`}>
                        <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
                            <h3 className={`font-bold text-lg ${textColor}`}>Add for {selectedDay?.toLocaleDateString('en-US', { weekday: 'long' })}</h3>
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            <div className="flex-1 overflow-y-auto max-h-60 space-y-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                {searchResults.map((recipe) => (
                                    ...
                                    >
                                        <img src={recipe.images?.[0]} className="w-10 h-10 rounded object-cover bg-gray-300" />
