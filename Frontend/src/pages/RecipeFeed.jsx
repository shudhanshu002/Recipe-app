...
const RecipeFeed = () => {
  ...
  return (
    <div
      className={`min-h-screen ${bgClass} pb-20 font-dancing mt-3 rounded-4xl`}
    >
      <ToastContainer />
      ...
      >
        <button
          onClick={() => navigate(-1)}
          className={`p-2 rounded-full transition-colors ${textColor} ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-200'}`}
        >
          <ArrowLeft size={24} />
        </button>
        <div>
        ...
        <div className="flex flex-1 overflow-hidden gap-6 relative">
          ...
          >
            {loading ? (
              ...
              <div
                className={`text-center py-40 rounded-xl border-2 border-dashed ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}
              >
