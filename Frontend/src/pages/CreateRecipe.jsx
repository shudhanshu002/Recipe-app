...
const CreateRecipe = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (files.length + images.length > 10)
      return toast.info('Max 10 images allowed');
      ...
      >
        ...
        {selectedCuisine === 'Other' && (
          <div className="animate-in fade-in slide-in-from-top-2">
            <Input
              label="Specify Cuisine Name"
              placeholder="e.g. Peruvian"
              {...register('customCuisine', {
                required: selectedCuisine === 'Other',
              })}
            />
          </div>
        )}
        ...
        <div className="flex flex-col gap-1">
          ...
          <textarea
            rows="3"
            className={`w-full px-4 py-2 rounded-lg border focus:outline-none ${inputBg}`}
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="text-sm text-[#f97316] flex items-center gap-1 mt-2"
            >
              <Plus size={16} /> Add
            </button>
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {previewUrls.map((url, i) => (
              <div
                key={i}
                className={`relative group aspect-square rounded-lg overflow-hidden border ${isDarkMode ? '' : 'border-gray-700'}`}
              >
                <img
                  src={url}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div
              className={`p-4 border-2 border-dashed rounded-xl text-center ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}
            >
              {videoPreview ? (
                ...
                <label
                  className={`cursor-pointer block py-8 transition-colors rounded-lg ${isDarkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}
                >
        ...
        <div
          className={`flex items-center gap-3 p-4 rounded-lg ${isDarkMode ? 'bg-yellow-900/20' : 'bg-yellow-50'}`}
        >
          <input
            type="checkbox"
            {...register('isPremium')}
            className="w-5 h-5 text-[#f97316] rounded"
          />
          <label
            className={`text-sm font-medium ${isDarkMode ? 'text-yellow-200' : 'text-yellow-800'}`}
          >
            Mark as Premium
          </label>
        </div>
