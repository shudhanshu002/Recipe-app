...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        const userRecipes = recipesRes.data.data.recipes.filter(
          (r) => r.createdBy?.username === username
        );
  ...
  const fetchUserList = async (type) => {
    ...
    try {
      const endpoint =
        type === 'followers'
          ? `/subscriptions/followers/${profile._id}`
          : `/subscriptions/following/${profile._id}`;
  ...
  const handleSubscribe = async () => {
    ...
    setProfile((prev) => ({
      ...prev,
      subscribersCount: !previousState
  ...
  if (error || !profile)
    return <div className={`text-center py-20 ${textColor}`}>{error}</div>;
  ...
  const UserListModal = ({ title, onClose, users }) => (
    <div className="font-dancing fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div
        className={`w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden ${cardBg}`}
      >
        <div
  ...
  return (
    <div className="font-dancing max-w-7xl mx-auto space-y-8 mb-16 px-4 md:px-6">
      ...
      <div className="relative px-2 md:px-8 -mt-24 md:-mt-28 z-20">
        <div
          className={`rounded-3xl p-6 md:p-8 shadow-xl border backdrop-blur-sm relative ${cardBg}`}
        >
          <div className="flex flex-col md:flex-row gap-6 md:gap-10">
            ...
            <div className="flex-1 w-full pt-2 text-center md:text-left">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <div>
                  <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                    <h1
                      className={`text-3xl md:text-4xl font-black tracking-tight ${textColor}`}
                    >
                      {profile.username}
                    </h1>
                  ...
                  {!isEditing && (
                    <p className={`text-sm ${subText} font-medium`}>
                      Joined Chef's Kitchen Community
                    </p>
                  )}
                </div>
              ...
              {isEditing ? (
                <div className="mt-4 p-5 bg-gray-50 dark:bg-black/20 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700 animate-in fade-in slide-in-from-top-2">
                  <div className="grid gap-4">
                    <div className="space-y-1">
                      <label
                        className={`text-xs font-bold uppercase ${subText}`}
                      >
                        Display Title
                      </label>
                      <input
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div
                        ...
                        onClick={() =>
                          setEditForm({
                            ...editForm,
                            isFollowingPublic: !editForm.isFollowingPublic,
                          })
                        }
                      >
                        <div
                          className={`w-5 h-5 rounded border flex items-center justify-center ${
                            editForm.isFollowingPublic
                ...
                <p
                  className={`${subText} text-base leading-relaxed max-w-3xl mb-6 mx-auto md:mx-0`}
                >
                  {profile.about ||
                    'This chef keeps their recipes spicy but their bio empty.'}
                </p>
              )}

              {/* Stats Row */}
              <div
                className={`flex flex-wrap justify-center md:justify-start gap-2 md:gap-8 pt-6 border-t ${isDarkMode ? 'border-gray-800' : 'border-gray-100'}`}
              >
                <button
                ...
                >
                  <span
                    className={`text-2xl font-black ${textColor} group-hover:text-[#f97316] transition-colors`}
                  >
                ...
                <div
                  className={`w-px h-12 self-center ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}
                />
                ...
                >
                  <span
                    className={`text-2xl font-black ${textColor} group-hover:text-blue-500 transition-colors`}
                  >
      ...
      <div className="px-2">
        ...
        {recipes.length > 0 ? (
          ...
          <div
            className={`text-center py-16 rounded-3xl border border-dashed ${isDarkMode ? 'border-gray-800 bg-[#1e1e1e]/50' : 'border-gray-200 bg-gray-50'}`}
          >
