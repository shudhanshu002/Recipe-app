...
const CommentThread = ({
  ...
  const isReplying =
    activeReplyBox.reviewId === reviewId &&
    activeReplyBox.parentId === reply._id;
          ...
          <div className={`p-3 rounded-lg ${replyBg}`}>
            <div className="flex justify-between">
              <Link
                ...
                className={`font-bold text-xs block mb-1 ${textColor} hover:underline`}
              >
            ...
            <div className="mt-2">
              {!showNested ? (
                <button
                  ...
                  className="text-xs font-bold text-[#f97316] flex items-center gap-1 ml-2 hover:underline"
                >
                ...
                <div className="mt-2">
                  ...
                  <button
                    ...
                    className="text-xs text-gray-400 flex items-center gap-1 mt-2 ml-4 hover:text-gray-600"
                  >
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full bg-linear-to-t from-black/90 via-black/40 to-transparent p-3 pt-6 pointer-events-none">
        <p className="text-white text-xs font-bold truncate mb-1 drop-shadow-md">
          {short.caption}
        </p>
...
const RecipeDetail = () => {
  ...
  const myReview = useMemo(
    () => reviews.find((r) => r.author?._id === user?._id),
    [reviews, user]
  );
  ...
  const handleAddToShoppingList = async () => {
    if (!user)
      return toast.error('Login to use shopping list', {
        position: 'top-right',
      });
    setAddingToCart(true);
    try {
      const res = await api.post('/shoppinglist/add-from-recipe', {
        recipeId: id,
      });
  ...
  const handleRateRecipe = async (rating) => {
    ...
    try {
      ...
      if (e.response?.status === 403)
        toast.success(
          e.response.data.message || 'You cannot rate your own recipe'
        );
  ...
  const handleUploadShort = async (e) => {
    ...
    if (file.size > 100 * 1024 * 1024)
      return toast.dismiss('File too large (Max 100MB)');
    setIsUploadingShort(true);
    const formData = new FormData();
    formData.append('video', file);
    formData.append('caption', `Short by ${user.username}`);
    try {
      await api.post(`/shorts/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  if (loading)
    return (
      <div
        className={`min-h-screen pt-20 pb-20 ${isDarkMode ? 'bg-[#121212]' : 'bg-gray-50'}`}
      >
  ...
  if (!recipe && !isLocked)
    return (
      <div className={`text-center py-20 ${textColor}`}>Recipe not found</div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 mb-20 font-dancing">
      ...
      <div className="space-y-4">
        <div
          className={`aspect-video w-full rounded-2xl overflow-hidden shadow-lg relative ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'} group`}
        >
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          ...
          <span
            className={`px-3 py-1 rounded-full border text-xs font-bold uppercase ${getDiffColor(recipe?.difficulty)}`}
          >
            {recipe?.difficulty}
          </span>
        </div>
        ...
        <div
          className={`flex flex-wrap justify-between items-center gap-4 py-4 border-y ${metaBorder} ${subText}`}
        >
          <div className="flex gap-6 items-center">
            <Link
              to={`/profile/${recipe?.createdBy?.username}`}
              className="flex items-center gap-2 hover:text-[#f97316] font-medium"
            >
              <img
          ...
          <span className="text-sm">
            Published: {formatRelativeDate(recipe?.createdAt)}
          </span>
        </div>
      </div>
      ...
      {!isLocked && recipe?.videoUrl && (
        <div
          className={`mt-6 pt-8 border-t ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}
        >
          <h3
            className={`text-xl font-bold mb-3 flex items-center gap-2 ${textColor}`}
          >
          ...
          <div className="aspect-video w-full rounded-xl overflow-hidden bg-black shadow-lg">
            <CustomVideoPlayer
              src={recipe.videoUrl}
              poster={recipe.videoThumbnail || recipe.images?.[0]}
            />
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 space-y-6">
          ...
          >
            ...
            {addingToCart ? 'Adding...' : 'Add to Shopping List'}
          </button>
        </div>
        <div className="md:col-span-2 space-y-6">
          ...
          {isLocked ? (
            <div
              className={`p-8 rounded-xl text-center border-2 border-dashed ${lockBoxBg}`}
            >
              <Lock className="mx-auto text-gray-400 mb-2" size={32} />
              <p className={subText}>Locked Content</p>
              <Link
                to="/subscription"
                className="text-[#f97316] hover:underline font-bold mt-2 inline-block"
              >
                Upgrade to Premium
              </Link>
            </div>
            ...
            <>
              ...
              <div className={`mt-8 pt-6 border-t ${metaBorder}`}>
                <h4 className={`text-lg font-bold mb-3 ${textColor}`}>
                  {isOwner ? 'Your Recipe Rating' : 'Rate this Recipe'}
                </h4>
                {isOwner ? (
                  <div className="flex items-center gap-2 opacity-50 cursor-not-allowed">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        ...
                        size={32}
                        className={
                          recipe.averageRating >= star
                            ? 'text-yellow-400 fill-yellow-400'
                            : 'text-gray-300'
                        }
                      />
                    ))}
                    <span className={`ml-2 text-sm ${subText}`}>
      ...
      <div className={`pt-8 border-t ${metaBorder}`}>
        <div className="flex justify-between items-center mb-4">
          <h3
            className={`text-2xl font-bold flex items-center gap-2 ${textColor}`}
          >
      ...
      <div className={`pt-8 border-t ${metaBorder}`}>
        <h3
          className={`text-2xl font-bold mb-6 flex items-center gap-2 ${textColor}`}
        >
        ...
        {user ? (
            ...
            <div className="flex-1 relative">
              ...
              <div className="flex items-center pr-2 gap-1 absolute right-2 top-2">
                ...
                <button
                  ...
                  className="p-2 bg-[#f97316] text-gray-400 rounded-lg"
                >
                  <Send size={18} />
                </button>
              </div>
          ...
          <p
            className={`mb-6 text-center py-4 rounded-lg ${subText} ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
          >
        ...
        <div className="space-y-2">
            ...
            return (
              <div key={review._id} className="group">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div
                      className={`p-4 rounded-xl rounded-tl-none ${isDarkMode ? 'bg-[#1e1e1e]' : 'bg-gray-100'}`}
                    >
                      <div className="flex justify-between mb-1">
                        <Link
                          to={`/profile/${review.author?.username}`}
                          className={`font-bold text-sm ${textColor} hover:underline`}
                        >
                    ...
                    <div className="flex items-center gap-4 mt-1 ml-2">
                      ...
                      <button
                        onClick={() =>
                          setActiveReplyBox(
                            activeReplyBox.reviewId === review._id &&
                              !activeReplyBox.parentId
                              ? { reviewId: null, parentId: null }
                    ...
                    {activeReplyBox.reviewId === review._id &&
                      ...
                      user && (
                        <div className="flex gap-2 mt-3 ml-2">
                          ...
                          />
                          <button
                            onClick={() => handleReply(review._id, replyText)}
                            className="px-3 py-1 bg-[#f97316] text-white rounded-lg text-xs"
                          >
                            Send
                          </button>
                      ...
                      <div className="mt-2">
                        {!expandedReplies[review._id] ? (
                          <button
                            ...
                            className="text-xs font-bold text-[#f97316] flex items-center gap-1 ml-2"
                          >
                          ...
                          <div className="mt-3">
                            ...
                            <button
                              onClick={() => toggleReplies(review._id)}
                              className="text-xs text-gray-400 flex items-center gap-1 mt-2 ml-4"
                            >
                              <ChevronUp size={14} /> Hide replies
                            </button>
                          </div>
          ...
          {reviews.length > visibleReviewsCount && (
            ...
            >
              Load more comments ({reviews.length - visibleReviewsCount}{' '}
              remaining)
            </button>
          )}
        </div>
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-linear-to-t from-black/90 via-black/50 to-transparent text-white pointer-events-none">
              <div
                ...
                onClick={() =>
                  (window.location.href = `/profile/${viewingShort.author?.username}`)
                }
              >
                <img
                  src={viewingShort.author?.avatar}
                  className="w-10 h-10 rounded-full border-2 border-white"
                  alt="Author"
                />
                <span className="font-bold text-sm hover:underline">
                  {viewingShort.author?.username}
                </span>
              </div>
