...
const POPULAR_CATEGORIES = [
  ...
  },
  {
    ...
    label: 'Desserts',
    image:
      'https://images.unsplash.com/photo-1551024601-bec78aea704b?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8ZGVzc2VydHxlbnwwfHwwfHx8MA%3D%3D',
    filter: 'dessert',
  },
  {
    id: 'healthy',
...
const CUISINE_CARDS = [
  {
    label: 'Indian',
    image:
      'https://images.unsplash.com/photo-1659716307017-dc91342ec2b8?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8aW5kaWFuZm9vZHxlbnwwfHwwfHx8MA%3D%3D',
  },
  {
    label: 'Italian',
    image:
      'https://images.unsplash.com/photo-1595295333158-4742f28fbd85?w=500&q=80',
  },
  {
    label: 'Chinese',
    image:
      'https://images.unsplash.com/photo-1585032226651-759b368d7246?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8Y2hpbmVzZSUyMGZvb2R8ZW58MHx8MHx8fDA%3D',
  },
  {
    label: 'Mexican',
    image:
      'https://images.unsplash.com/photo-1626808642875-0aa545482dfb?w=500&q=80',
  },
  {
    label: 'American',
    image:
  ...
  },
  {
    label: 'Japanese',
    image:
      'https://plus.unsplash.com/premium_photo-1664205766166-6751c4c65f53?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8amFwYW5lc2Vmb29kfGVufDB8fDB8fHww',
  },
  {
    label: 'Mediterranean',
    image:
      'https://images.unsplash.com/photo-1511690656952-34342bb7c2f2?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8TWVkaXRlcnJhbmVhbiUyMGZvb2R8ZW58MHx8MHx8fDA%3D',
  },
  {
    label: 'Thai',
    image:
...
const CIRCLE_CATEGORIES = [
  {
    ...
    label: 'Quick & Easy',
    image:
      'https://images.unsplash.com/photo-1626808642875-0aa545482dfb?auto=format&fit=crop&w=300&q=80',
    param: 'maxTime=15',
  },
  {
    id: 'dinner',
...
const Home = () => {
  ...
  const handleVideoClick = (recipe) => {
    const isUserPremium =
      user?.subscriptionStatus === 'premium' || user?.isPremium === true;
    if (recipe.isPremium && !isUserPremium) {
      toast.error('Premium Content! Upgrade to watch.', {
        position: 'top-right',
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen pb-20 relative">
      ...
      <div className="text-center py-16 px-4 space-y-6 relative overflow-hidden">
        ...
        <p
          className={`font-recur font-bold text-4xl md:text-xl max-w-2xl mx-auto ${isDarkMode ? 'text-gray-400' : 'text-black'}`}
        >
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-24">
        ...
        <section>
          <div className="flex justify-between items-end mb-6">
            <h2
              className={`font-dancing text-2xl md:text-3xl font-bold ${textColor}`}
            >
              Popular Categories
            </h2>
          </div>
              ...
              : POPULAR_CATEGORIES.map((cat) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-6 left-6">
                      <h3 className="font-dancing text-2xl font-bold text-white group-hover:text-[#f97316] transition-colors">
                        {cat.label}
                      </h3>
                    </div>
                  </MotionCard>
        ...
        {loading ? (
          ...
          <section className={`py-8 -mx-4 px-4 font-dancing`}>
            <div className="max-w-7xl mx-auto">
              <div className="flex items-center gap-3 mb-6">
                <div
                  className={`p-2 rounded-lg text-red-500 ${isDarkMode ? 'bg-red-900/30' : 'bg-red-100'}`}
                >
                  <PlayCircle size={24} />
                </div>
          ...
          <div
            className={`p-10 rounded-3xl border-2 border-dashed text-center ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}
          >
            <p className={subText}>
              No favorite videos yet. Like a recipe with a video to see it here!
            </p>
          </div>
        )}
        {/* 3. TRENDING RECIPES */}
        <section className="font-dancing">
          <div className="flex items-center gap-3 mb-6">
            <div
              className={`p-2 rounded-lg text-yellow-600 ${isDarkMode ? 'bg-yellow-900/30' : 'bg-yellow-100'}`}
            >
        ...
        <section className="font-dancing">
          <h3
            className={`text-xl font-bold mb-6 ${textColor} flex items-center gap-2`}
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {TIME_CARDS.map((item) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${item.color} group-hover:scale-110 transition-transform`}
                >
                  {item.icon}
                </div>
        ...
        <section className="font-dancing">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-4">
            {CUISINE_CARDS.map((c) => (
              <div
                key={c.label}
                onClick={() => handleFilterClick('cuisine', c.label)}
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/40 to-transparent flex flex-col justify-center px-10 md:px-20">
                <span className="bg-yellow-500 text-black font-extrabold px-4 py-1.5 rounded-full text-xs w-fit mb-6 animate-pulse uppercase tracking-wide">
                  ⭐ Recipe of the Day
                </span>
                <h2 className="text-5xl md:text-7xl font-black text-white mb-4 leading-tight max-w-2xl">
                  {recipeOfTheDay.title}
                </h2>
                <p className="text-gray-200 mb-8 max-w-lg text-lg leading-relaxed line-clamp-2">
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() => handleRecipeClick(recipeOfTheDay)}
            ...
            <div
              className={`w-full h-full flex items-center justify-center text-gray-500 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}
            >
              No recipe of the day available.
            </div>
          )}
        </section>
        {/* 7. LATEST RECIPES */}
        <section className="font-dancing">
          <div className="flex items-center gap-3 mb-6">
            <div
              className={`p-2 rounded-lg text-green-600 ${isDarkMode ? 'bg-green-900/30' : 'bg-green-100'}`}
            >
        ...
        <section
          className={`py-16 -mx-4 px-4 ${isDarkMode ? 'bg-[#1a1a1a]' : 'bg-gray-100'} font-dancing rounded-4xl relative group`}
        >
          <div className="max-w-7xl mx-auto">
            <h2 className={`text-3xl font-bold mb-10 text-center ${textColor}`}>
              Meet Our Top Chefs
            </h2>
            ...
            {topChefs.length > 0 ? (
                    ...
                    >
                      <div className="relative">
                        <img
                          src={
                            chef.avatar ||
                            `https://ui-avatars.com/api/?name=${chef.username}&background=random`
                          }
                      ...
                      <div>
                        <h4
                          className={`font-bold text-lg ${textColor} truncate max-w-[120px]`}
                        >
                          {chef.username}
                        </h4>
              ...
              <div
                className={`text-center  py-10 ${isDarkMode ? 'text-gray-500' : 'text-gray-900'}`}
              >
                <p>
                  No top chefs found yet. Start creating recipes to be here!
                </p>
              </div>
            )}
          </div>
        ...
        <section className="py-16 px-4 font-dancing">
          <div
            className={`max-w-6xl mx-auto rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row ${isDarkMode ? 'bg-[#1a1a1a]' : 'bg-white'}`}
          >
            <div className="flex-1 p-10 md:p-16 flex flex-col justify-center text-left relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center text-orange-500 mb-6 ${isDarkMode ? 'bg-orange-900/30' : 'bg-orange-100'}`}
                >
                  <Mail size={24} />
                </div>
                <h2
                  className={`text-4xl font-bold mb-4 font-sans ${textColor}`}
                >
                  Hungry for updates?
                </h2>
                <p className={`text-lg mb-8 ${subText}`}>
                  Get our "Chef's Special" newsletter delivered every Friday.
                </p>
          ...
          >
            <div className="aspect-video w-full">
              <CustomVideoPlayer
                ...
                poster={viewingVideo.videoThumbnail || viewingVideo.images?.[0]}
                autoPlay={true}
              />
            </div>
            <button
