...
const getAllBlogs = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, blogsWithCounts, 'Blogs fetched'));
});
...
const getBlogById = asyncHandler(async (req, res) => {
  ...
  const blog = await Blog.findByIdAndUpdate(id, updateQuery, {
    new: true,
  }).populate('author', 'username avatar about');
...
const toggleBlogReaction = asyncHandler(async (req, res) => {
  ...
  if (existingIndex > -1) {
    if (blog.reactions[existingIndex].emoji === emoji)
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, blog.reactions, 'Reaction updated'));
});
...
const getComments = asyncHandler(async (req, res) => {
  ...
  const comments = await Comment.find({ blog: id })
    .populate('author', 'username avatar')
    .sort({ createdAt: 1 }); // Sorted oldest to newest for tree building
  return res
    .status(200)
