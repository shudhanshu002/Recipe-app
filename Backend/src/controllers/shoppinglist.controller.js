...
const getShoppingList = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, list, 'Shopping list fetched'));
});
...
const addShoppingItem = asyncHandler(async (req, res) => {
  ...
  if (existingItem) {
    return res
      .status(200)
      .json(new ApiResponse(200, list, 'Item already in list'));
  }
...
const addFromRecipe = asyncHandler(async (req, res) => {
  ...
  recipe.ingredients.forEach((ing) => {
    ...
    const exists = list.items.some(
      (item) => item.ingredient.toLowerCase() === ing.toLowerCase()
    );
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, list, `Added ${addedCount} ingredients to your list`)
    );
});
