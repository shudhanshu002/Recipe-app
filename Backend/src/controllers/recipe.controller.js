...
const getRecipeById = asyncHandler(async (req, res) => {
  ...
  if (userId) {
    ...
    if (!alreadyViewed) {
      ...
      recipe = await Recipe.findById(id).populate(
        'createdBy',
  ...
  if (recipe.isPremium) {
    ...
    const isCreator =
      req.user &&
      recipe.createdBy &&
      req.user._id.toString() === recipe.createdBy._id.toString();
...
const getAllRecipes = asyncHandler(async (req, res) => {
  ...
  if (cuisine && cuisine !== 'All' && cuisine !== 'null')
  ...
  if (userId) {
    ...
    bookmarkedRecipeIds = new Set(
      userBookmarks.map((b) => b.recipe.toString())
    );
  }
...
const getUserLikedVideos = asyncHandler(async (req, res) => {
  ...
  const recipes = await Recipe.find({
    ...
    .select(
      'title videoUrl videoThumbnail images cookingTime calories isPremium isVegetarian'
    )
    .populate('createdBy', 'username avatar');

  return res
    .status(200)
    .json(new ApiResponse(200, recipes, 'Liked videos fetched successfully'));
});
...
const getTopChefs = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, topChefs, 'Top chefs fetched'));
});
...
const getRecipeOfTheDay = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, recipe[0], 'Recipe of the Day'));
});
