...
const getUserFollowing = asyncHandler(async (req, res) => {
    ...
    if (!isMe && !user.isFollowingPublic) {
        return res.status(403).json(new ApiResponse(403, null, 'Following list is private'));
    }

    const following = await Subscription.find({ subscriber: subscriberId }).populate('channel', 'username avatar title').select('channel');
    return res.status(200).json(
        new ApiResponse(
            200,
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (ratedRecipes.length > 0) {
            const totalRating = ratedRecipes.reduce((acc, curr) => acc + curr.averageRating, 0);
    ...
    const badges = [];
    if (totalLikes > 10) badges.push({ label: 'Top Chef', icon: '⭐', color: 'yellow' });
...
export { toggleSubscription, getUserChannelProfile, getUserFollowers, getUserFollowing };
